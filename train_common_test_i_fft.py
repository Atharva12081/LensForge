...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an FFT radial-feature baseline for Common Test I.")
    ...
    parser.add_argument("--report-path", type=Path, default=Path("reports/common_test_i_fft.json"))
    return parser.parse_args()


def fft_radial_features(image: np.ndarray, bins: int) -> np.ndarray:
...
def load_split(root: Path, split: str, per_class: int, bins: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for label, class_name in enumerate(CLASSES):
        ...
        for path in files:
            ...
            features.append(fft_radial_features(image, bins=bins))
            labels.append(label)
